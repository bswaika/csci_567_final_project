...
if __name__ == '__main__':
    ...
    if is_forced() or not is_files_exist_in(DATA_DIR, ['customers', 'articles', 'master']):
        ...
        transactions_df = select_columns(transactions_df, ['customer_id', 'article_id', 't_dat', 'price'])
        ...
        # Clean user data
        users_df = users.fill(users_df)

        # Generate item keywords and select appropriate columns
        items_df = items.generate_keywords(items_df)
        items_df = select_columns(items_df, ['article_id', 'prod_name', 'keywords'])
        ...
        master_df = transactions_df.merge(items_df, on='article_id')
        master_df = master_df.merge(users_df, on='customer_id')

        # Write master dataframe to disk
        save_df(master_df, DATA_FILES['master'])
